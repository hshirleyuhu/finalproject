...
var svg = d3.select('#slope-graph').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
function ready(datapoints) {
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 3).attr('cx', function (d) {
    ...
    var className = d.state.toLowerCase().replace(/[^a-z]*/g, '');
    svg.selectAll('path.' + className).attr('stroke', 'red').raise();
    svg.selectAll('circle.' + className).attr('fill', 'red').raise();
    svg.selectAll('text.' + className).attr('fill', 'red').raise();
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('stroke', function (d) {
    ...
    var className = d.key.toLowerCase().replace(/[^a-z]*/g, '');
    svg.selectAll('path.' + className).attr('stroke', 'red').raise();
    svg.selectAll('circle.' + className).attr('fill', 'red').raise();
    svg.selectAll('text.' + className).attr('fill', 'red').raise();
  }).on('mouseout', function (d) {
