...
var xPositionScale = d3.scaleLinear().domain([0, 24]).range([0, width]);
...
d3.csv(require('../data/hourcount.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.append('path').datum(datapoints).attr('d', line).attr('stroke', '#A9A9A9').attr('stroke-width', 2).attr('fill', 'none');
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('cx', function (d) {
    return xPositionScale(d.Hour);
  }).attr('cy', function (d) {
    return yPositionScale(d.percentage);
  }).attr('r', 3).attr('fill', '#A9A9A9').on('mouseover', function (d) {
  ...
  svg.append('text').text('Busiest Hour of A Day').attr('x', 95).attr('y', -20).attr('text-anchor', 'middle').attr('font-size', 22).attr('font-weight', 'bold');
  var xAxis = d3.axisBottom(xPositionScale).tickValues([0, 4, 8, 12, 16, 20, 24]) // .tickFormat(d3.timeFormat('%b %Y'))
  .ticks(5);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  var yAxis = d3.axisLeft(yPositionScale).tickValues([2, 4, 8, 10]).tickSize(-width);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis);
  svg.selectAll('.y-axis path').remove();
  svg.selectAll('.y-axis line').attr('stroke-dasharray', 2).attr('stroke', 'grey');
}
},{"d3":"../node_modules/d3/index.js","../data/hourcount.csv":"data/hourcount.csv"}]},{},["scripts/02-line-chart.js"], null)
